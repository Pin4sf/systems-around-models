...
revision_id: revision-qm-scoped-resources-and-leased-runs-002
...
## Declare adapter capabilities instead of promising parity

QM's adapter interface makes several differences explicit. Each adapter declares its control transport, tool transport, transcript format, and a set of capabilities such as abort, steer, images, thinking level, fast mode, and provider sessions. Tool names also belong to the adapter presentation layer: one adapter can expose `read` while another presents the same core operation as `workspace_read`.

This is a more honest compatibility contract than a single `supportsHarness` flag. A stable outer turn interface can coexist with real differences in control, tools, transcripts, and recovery. Selection should therefore compare the request with a declared profile, reject unsupported combinations, and preserve the chosen profile in the run record. A missing capability should become a typed degraded state or an admission failure—not a prompt that asks the model to pretend the capability exists.

Profiles also make conformance testable. For each adapter, test the same identity and grant snapshot, tool-name mapping, cancellation path, transcript reconstruction, and reset-on-switch behavior. Passing the common cases does not prove semantic parity; it establishes exactly which contract each adapter satisfies.
...
The same limit applies to recorded tool results. A run, attempt, and call index can identify a reusable result inside the harness's replay contract. It cannot prove that an external effect with no durable result happened zero or one times. Replay coordinates organize evidence; effect reconciliation establishes what the world accepted.
