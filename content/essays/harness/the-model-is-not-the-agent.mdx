...
Atlas is a fictional software-delivery agent investigating duplicate payments after provider timeouts. The following is a synthetic failure trace derived from an observed failure class. It is not a documented incident at a named company.
...
That broader pattern—an uncertain external result followed by a blind retry—can duplicate an effect even when every individual call looks locally reasonable.

<EvidenceBadge
  claimId="claim-ambiguous-effect-failure-pattern"
  sectionAnchor="failure-before-definition"
...
<EvidenceBadge
  ...
  sectionAnchor="the-harness-boundary"
...
None of those properties follows automatically from where the process runs. Local execution may improve custody while still exposing broad credentials or hiding state in an opaque store. A provider selector may offer model choice while sessions, capabilities, and recovery remain locked to one provider's semantics. Open source may improve inspectability while a hosted deployment still controls the user's data. Ownership, privacy, security, and independence are separate claims that need separate evidence.

Earendil argues that people can own and adapt their harness, choose among model providers through its translation layer, and retain local copies of sessions. That is the source's author-reported ownership philosophy; it is not evidence that every harness delivers those properties.

<EvidenceBadge
  claimId="claim-earendil-harness-ownership-philosophy"
  sectionAnchor="control-and-user-leverage"
...
<EvidenceBadge
  ...
  sectionAnchor="the-end-to-end-trace"
...
<EvidenceBadge
  ...
  sectionAnchor="what-architecture-selection-changes"
...
This chapter proposes a way to inspect agent systems, not a final vocabulary for the field.

First, “harness” is not a settled industry standard. Boundaries vary across sources and implementations. A narrower definition is not automatically wrong; it becomes dangerous only when responsibilities outside that definition are treated as if they disappeared.

Second, Earendil's four-part framing is a beginner map, not a complete production contract. The production extensions in this chapter are this publication's synthesis. They should not be attributed back to Earendil.

Third, stronger models may retire some capability-bearing scaffolding. Better native tool use, longer context, or more reliable planning can simplify parts of a harness. They do not automatically remove governance-bearing controls such as credential scope, effect identity, independent Verification, approval, revocation, and durable recovery.

Fourth, portability claims require provider-specific conformance tests. A shared interface demonstrates common syntax. It does not by itself demonstrate equal event semantics, session recovery, approval behavior, capability scope, or degraded-mode operation.

Fifth, user ownership must be demonstrated through custody, export, revoke, delete, and degraded-mode behavior. A local process or a model dropdown is evidence for only part of that claim.

The central configuration thesis is falsifiable. A strong counterexample would be:

> A model-only evaluation reliably predicts deployed agent performance across materially different harnesses, environments, and authority boundaries.

If that result held under careful, repeated evaluation, the model × harness × environment × authority formulation would need to shrink or change. Specific architecture claims should carry narrower falsifiers: a conformance suite may disprove a claimed provider gap; a recovery test may show that state survives a failure we expected to lose; a custody audit may show that a supposedly local system depends on remote state. The point is to make revision possible, not to protect the thesis from evidence.

## Sources and Revision Record

This chapter uses two public source records.

1. [What is a Harness?](https://earendil.com/posts/what-is-a-harness/) by Earendil supplies the attributed beginner-oriented four-part framing and the source author's ownership philosophy. This publication does not treat it as proof of provider parity, authorization enforcement, or external-effect success.
2. [The Agent Systems Fieldbook — Harness Engineering, version 1](https://github.com/Pin4sf/systems-around-models/blob/main/content/essays/harness/the-model-is-not-the-agent.mdx) records this publication synthesis and its provenance. It is not independent proof of the central inferred claim.

Evidence labels describe the state of a specific claim, not the prestige of a source. Source evidence grades describe the basis available in that source record. The interface keeps those classifications separate: `observed` is not another name for `IP`, and a first-party document does not become implementation evidence through presentation.

The revision record at the end of this page names the public release, the affected claims, and the path for corrections. A correction should identify the exact route, revision, section, and claim; provide public counter-evidence; and omit credentials, personal data, private traces, and copyrighted documents.
